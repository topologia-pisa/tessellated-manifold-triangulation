...
from tessellation import (
    ...
    Tessellated_manifold_isometry,
    Tessellated_manifold_isometry_group,
)
from regina import Perm4
from P5 import P5, P5_facet, P5_iso, I, J, K, rt
...
def get_S5():
    ...
    def pasting_map_sphere(facet: P5_facet):
        p = facet.pol
        manifold = p.manifold
        id = P5_iso()
...
def get_M5():
    ...
    return Tessellated_manifold(
        P5, [x for x in itertools.product(*[[0, 1]] * 8)], pasting_map_M5, facet_state
    )
...
q8_iso = {
    1: P5_iso(),
    -1: P5_iso([-1, -1, -1, -1]),
    I: P5_iso([1, -1, 1, -1], Perm4(3, 2, 1, 0)),
    J: P5_iso([1, -1, -1, 1], Perm4(1, 0, 3, 2)),
}
...
def get_minimal_quotient():
    ...
    def pasting_map_minimal_quotient(facet):
        ...
        if level % 2 == 0:
            if level == 2 and facet.state:
                return None
            iso = q8_iso[
                {1: 1, -1: 1, I: J, -I: J, J: K, -J: K, K: I, -K: I}[facet.label]
            ]
            new_p = manifold.polytopes[
                (
                    level + (1 if facet.state else -1),
                    rotation * (I if facet.index[4] == 1 else -I),
                )
            ]
            return (new_p, iso)
            ...
            if level == -1 and not facet.state:
                return None
            iso = q8_iso[
                {1: 1, -1: 1, I: -K, -I: -K, J: -I, -J: -I, K: -J, -K: -J}[facet.label]
            ]
            new_p = manifold.polytopes[
                (
                    level + (1 if facet.state else -1),
    ...
    mnf = Tessellated_manifold(
        P5,
        [(-1, I), (-1, -I), (0, 1), (0, -1), (1, I), (1, -I), (2, 1), (2, -1)],
        pasting_map_minimal_quotient,
        facet_state,
    )
    isom = Tessellated_manifold_isometry(
        mnf, mnf.polytopes[(0, 1)], mnf.polytopes[(1, I)], quotienting_iso
    )
...
def get_M5_cyclic_covering():
    ...
    return Tessellated_manifold(
        P5,
        [x for x in itertools.product(*[[0, 1]] * 8 + [[1, 0, 2]]) if sum(x) % 2 == 0],
        pasting_map_leveled_M5,
        facet_state,
    )
...
def get_M5_two_quotient():
    ...
    isometries = [
        Tessellated_manifold_isometry(
            leveled_M5, leveled_M5.polytopes[(0,) * 9], leveled_M5.polytopes[x], y
        )
        for x, y in [
...
def get_rt_minimal_manifold():
    ...
    def pasting_map_rt(facet: P5_facet):
        ...
        m = (
            get_matrix(number, target_number)
            if str(number) in rt["pasting_matrices"]
            else get_matrix(target_number, number).inverse()
        )
        try:
